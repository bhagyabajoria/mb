...
export class CustomDiseaseAPI {
  ...
  async searchDiseases(query: string, limit: number = 10): Promise<CustomDisease[]> {
    ...
    const queryWords = searchTerm.split(/\s+/).filter(word => word.length > 2);
  ...
  async searchBySymptoms(symptoms: string[], limit: number = 15): Promise<CustomDisease[]> {
    ...
    return this.diseases
      .map(disease => {
        const matchingSymptoms = disease.symptoms.filter(symptom =>
          normalizedSymptoms.some(userSymptom => {
            const symptomLower = symptom.toLowerCase();
            return symptomLower.includes(userSymptom) || 
        ...
        return {
          ...
          matchPercentage: (matchingSymptoms.length / normalizedSymptoms.length) * 100
        };
      })
      ...
      .slice(0, limit)
      .map(({ matchingSymptoms, matchCount, matchPercentage, ...disease }) => disease);
  }
